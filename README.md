...
当你推送 `v*` 标签（如 `v1.0.0`）时，会自动：

- 创建 GitHub Release（自动生成 Release Notes）
- 推送 Docker 版本标签（如 `v1.0.0`、`1.0.0`、`1.0`）
...
倉庫已包含：`.github/workflows/release-on-tag.yml`。  
當你推送 `v*` 標籤（例如 `v1.0.0`）時，會自動：

- 建立 GitHub Release（自動產生 Release Notes）
- 推送 Docker 版本標籤（例如 `v1.0.0`、`1.0.0`、`1.0`）
...
#### Version Release Flow (`v1.0.0`)

This repository also includes `.github/workflows/release-on-tag.yml`.  
When you push a `v*` tag (for example `v1.0.0`), it will automatically:

- Create a GitHub Release with autogenerated release notes
- Publish Docker version tags (for example `v1.0.0`, `1.0.0`, `1.0`)
...
このリポジトリには `.github/workflows/release-on-tag.yml` も含まれています。  
`v*` タグ（例: `v1.0.0`）を push すると自動で:

- GitHub Release を作成（Release Notes 自動生成）
- Docker のバージョンタグを公開（例: `v1.0.0`, `1.0.0`, `1.0`）
